...
bool Model::loadModel(char* filename)
{
	...
	{
		Vector3 currentvert = verts[faces[i] - 1];
		Vector3 currentnorm = norms[faces[+2] - 1];
		Vector3 currenttext = texCs[faces[i + 1] - 1];

		vertex.push_back(currentvert.x);
		vertex.push_back(currentvert.y);
		vertex.push_back(currentvert.z);

		normals.push_back(currentnorm.x);
		normals.push_back(currentnorm.y);
		normals.push_back(currentnorm.z);

		texCoords.push_back(currenttext.x / uMax);
		texCoords.push_back(currenttext.y / vMax);
	}
